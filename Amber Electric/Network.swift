...
class AmberAPI {
    ...
    private var timer : Timer?  //every 30 mins at xx:00:05 and xx:15:05
    ...
    func update(completionHandler : ((_ result: NetworkResult) -> Void)?) {
        ...
        /* 15 minute timer */
        if timer == nil {
            timer = Timer(fire: Date().nextMinutes(minutes: 30, plusSeconds: 5), interval: 60*15, repeats: true, block: { (_) in
    ...
    private func login(completionHandler : ((_ result: NetworkResult) -> Void)?) {
        ...
        /* Firstly check cached currentPriceData for a result in the current time block and return that if appropriate (avoid unnecessary network calls). Assumes data does not change in each 30 minute time period. */
        if let currentCachedTime = currentPriceData?.data.currentPricePeriod {
            let nextValidTimePeriod = currentCachedTime.nextMinutes(minutes: 30)
