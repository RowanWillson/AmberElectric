...
class ExtensionDelegate: NSObject, WKExtensionDelegate {
    ...
    func applicationDidFinishLaunching() {
        ...
        WKExtension.shared().scheduleBackgroundRefresh(withPreferredDate: Date().nextMinutes(minutes: 15, plusSeconds: 15), userInfo: nil) { (error) in
        ...
        for task in backgroundTasks {
            ...
            case let backgroundTask as WKApplicationRefreshBackgroundTask:
                AmberAPI.shared.update { (result) in
                    ...
                    WKExtension.shared().scheduleBackgroundRefresh(withPreferredDate: Date().nextMinutes(minutes: 15, plusSeconds: 15), userInfo: nil, scheduledCompletion: { (_) in
